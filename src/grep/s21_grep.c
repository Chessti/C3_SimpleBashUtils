...
void parse_arguments(int argc, char *argv[], grepArguments *arguments);
void handle_option(int opt, grepArguments *arguments);
void grep(char *filename, int files_counter, grepArguments arguments);
regex_t *compile(grepArguments arguments);
int print(int reg, int line_number, char *line, int *counter, int files_counter,
          char *filename, grepArguments arguments, regex_t *regex, FILE *fp);
int count_files(int argc, char *argv[], grepArguments arguments);
void flag_o(regex_t *regex, char *word, FILE *fp, grepArguments arguments);
void flag_h(int files_counter, char *filename, grepArguments arguments);
void get_pattern_f(char *filename, grepArguments *arguments);
void clean(grepArguments arguments, regex_t *regex, FILE *fp);

int main(int argc, char *argv[]) {
  grepArguments arguments = {0};
...
void handle_option(int opt, grepArguments *arguments) {
  switch (opt) {
    ...
    case 'f':
      arguments->f = 1;
      get_pattern_f(optarg, arguments);
      break;
    case 'o':
      arguments->o = 1;
      break;
